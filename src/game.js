...
export function runGame(canvas, ctx) {
  ...
  //const [player, sprites ] = R.partition(x => x.type === "player", map());
   const player = newPlayer(0, 0);
   const sprites = R.range(-10, -6).map(i => newSprite(i, i, "#"));
  ...
  state = {
    ...
    spriteIndex,
    setState,
    canvasOffset: canvasOffset(canvas),
  };
...
function drawGame({ player, sprites, canvas, ctx, canvasOffset }) {
    ctx.reset();
    drawGrid(canvas, ctx);
